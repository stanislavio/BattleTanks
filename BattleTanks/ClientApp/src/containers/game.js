...
class GameWrapper extends Component {
  ...
  componentWillUnmount() {
    ...
    const coor = map.map((x) => x.join(",")).join("|");
    this.props.saveGameMapData({
      Id: this.props.match.params.gameId,
      CurrentMap: coor,
    });
    this.props.saveGameData(
      this.props.game.players.data.map((el) => ({
        Id: el.info.id,
        Position:
          '{"x": ' +
          el.center_x +
  ...
  render() {
    ...
    const content =
      players.isSuccess & map.isSuccess ? (
        <Game
          rebuild_user={() => {
            this.props.rebuild_user(this.props.user.id);
          }}
          setPlayers={() => {
            if (this.props.game.players.data.length < 2)
              this.props.set_players(this.props.match.params.gameId);
          }}
          setFirstPlayer={this.setFirstPlayer}
          setSecondPlayer={this.setSecondPlayer}
          gameId={this.props.match.params.gameId}
          hub={this.props.hubConnection}
    ...
    const gameOver =
      data != null && data.winnerId != null ? <div>Game Over</div> : null;
    ...
    const renderPlayerInfo = (player) => {
      ...
      if (player.info != null) {
        return (
          <div>
            {player.info.nickname} {renderLives(player.lives)}
          </div>
        );
      }
      return <div></div>;
...
const mapDispatchToProps = (dispatch) => {
  return {
    rebuild_user: (id) => dispatch(get_user(id)),
