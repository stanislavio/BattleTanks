...
export default class Game extends Component {
  ...
  componentWillMount() {
    ...
    this.props.setFirstPlayer(
      players.find((el) => el.info.id == this.props.current_user.id)
    );
    this.props.setSecondPlayer(
      players.find((el) => el.info.id != this.props.current_user.id)
    );
    ...
    for (var i = 0; i < players.length; i++) {
      if (
        players[i].info.online == false &&
        players[i].info.id != this.props.current_user.id
      ) {
        this.setOnline(false);
        break;
      }
    ...
    if (this.props.hub != null) {
      ...
      this.props.hub.on("ReceiveOnline", (data) => {
        ...
        if (player != null) player.info.online = true;
      });
      ...
      this.props.hub.on("ReceiveShoot", (data) => {
        ...
        if (bullet_player != null) {
          ...
          bullet_player.enemies = players.filter(
            (el) => el.info.id != data.ownerId
          );
          bullet_player.position(
            JSON.parse(data.position).x,
            JSON.parse(data.position).y
          );
      ...
      this.props.hub.on("ReceiveMove", (data) => {
        ...
        if (current_player != null) {
          ...
          current_player.enemies = players.filter(
            (x) => x.info.id != data.ownerId
          );
  ...
  render() {
    ...
    players = players.data.map((x) => {
      if (x.x < 0 || x.y < 0) {
        ...
        this.props.hub.invoke("FirstPosition", {
          Id: x.info.id,
          Position: '{"x": ' + x.center_x + ', "y": ' + x.center_y + "}",
          Players: players.data
            .filter((x) => x.info.id != current_player.info.id)
            .map((x) => x.info.id)
            .join(","),
        });
      } else {
          ...
          <>
            <KeyboardEventHandler
              ...
              onKeyEvent={(key, e) => {
                if (
                  key == "space" &&
                  current_player.last_shoot + current_player.recharge_time <
                    new Date().getTime()
                ) {
                  ...
                  bullet.enemies = players.filter(
                    (el) => el != current_player && !el.died
                  );
                  ...
                  this.props.hub.invoke("Shoot", model).catch((err) => {
                    console.log("error");
                    console.log(err);
                  });
                  ...
                  this.props.hub.invoke("Move", model).catch((err) => {
                    console.log("error");
                    console.log(err);
                  });
                  current_player.enemies = players.filter(
                    (x) => x.info.id != current_user.id
                  );
                      ...
                      if (Object.keys(b).includes("enemy")) {
                        players = players.map((x) => {
                          if (x == b.enemy) {
                            ...
                            if (current_player.info.id != x.info.id) {
                              this.props.hub
                                .invoke("Kill", model)
                                .catch((err) => {
                                  console.log("error");
                        ...
                        this.props.setSecondPlayer(
                          players.find(
                            (el) => el.info.id != current_player.info.id
                          )
                        );
                      }
    ...
    const game = this.state.game ? (
      <Countdown date={this.state.connectedTime + 5000} renderer={renderer} />
    ) : (
      <div className="game-pause">Wait for other players</div>
    );
    ...
    return (
      <>
        {!this.state.gameOver ? (
          ...
          <div className="game-pause">Game Over</div>
        )}
      </>
    );
  }
